...
public class Utility {
	...
	/*	解析和处理服务器返回的省级数据*/
	public synchronized static boolean handleProvincesResponse(SevenWeatherDB sevenWeatherDB,String response){
		if(!TextUtils.isEmpty(response)){
			...
			if(allProvinces!=null&&allProvinces.length>0){
				for(String p:allProvinces){
					...
					sevenWeatherDB.saveProvince(province);
				}
				return true;
			}
		}
	...
	/*	解析和处理服务器返回的市级数据*/
	public static boolean handleCityResponse(SevenWeatherDB sevenWeatherDB,String response,int provinceId){
		if(!TextUtils.isEmpty(response)){
	...
	/*	解析和处理服务器返回的县级数据*/
	public static boolean handleCountyResponse(SevenWeatherDB sevenWeatherDB,String response,int cityId){
		if(!TextUtils.isEmpty(response)){
			...
			if(allCounties!=null&&allCounties.length>0){
				for(String c : allCounties){
					...
					//将解析出来的数据存储到County表
					sevenWeatherDB.saveCounty(county);		
				}
				return true;
			}
		}
	...
	/*	解析处理服务器返回的数据，并将解析出的数据存储到本地*/
	public static void handleWertherResponse(Context context,String response){
		try {
